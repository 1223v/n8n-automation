# n8n Orchestration Kit for Claude Code
...
1. **Claude Code** (최신 버전)
2. **n8n MCP 서버** — Claude Code의 MCP 설정(`~/.claude/mcp.json` 또는 프로젝트의 `.mcp.json`)에 아래와 같이 등록합니다. [`n8n-mcp`](https://www.npmjs.com/package/n8n-mcp) 패키지가 `n8n_list_workflows`, `n8n_get_workflow`, `n8n_create_workflow`, `n8n_update_partial_workflow`, `n8n_update_full_workflow`, `n8n_validate_workflow`, `n8n_autofix_workflow`, `n8n_test_workflow`, `validate_node` 등의 도구를 제공합니다.
...
   - **주의: API 키는 절대 이 README나 저장소에 커밋하지 마세요.** `mcp.json`은 개인 설정 파일이라 각자 자신의 키로 직접 채워 넣어야 합니다.
3. **context7 MCP 서버** — 최신 n8n 공식 문서 조회용 (리서처 에이전트가 사용)
4. 로컬에서 n8n을 직접 띄워 테스트하려면 Mac 또는 Windows PC (env-setup 스킬이 각각 지원)

## 설치 방법

1. 이 zip을 프로젝트 루트에 압축 해제합니다. 이미 `.claude/agents`, `.claude/skills`, `.claude/commands`가 있는 프로젝트라면 폴더를 병합(merge)하면 됩니다 — 다른 이름의 스킬/에이전트와 충돌하지 않습니다.
2. 프로젝트의 `CLAUDE.md`에 아래 트리거 포인터를 추가하세요 (선택이지만 강력히 권장 — 없어도 스킬 description으로 자동 트리거되지만, 명시해두면 더 안정적입니다):
...
3. (로컬 n8n을 직접 띄워 쓸 경우) `.claude/skills/n8n-env-setup/paths.env.example`을 참고해 필요하면 `paths.env`를 직접 만들어 채워도 되고, 그냥 두면 첫 실행 때 자동 탐지 후 알아서 채워줍니다.

## 빠른 시작

```
사용자: n8n 켜줘
→ n8n-env-setup 스킬이 현재 OS(Mac/Windows)를 감지해 로컬 n8n + Playwright MCP를 기동

사용자: 결제 알림 워크플로우에 Slack 알림 노드 추가해줘
→ n8n-workflow-manager가 워크플로우 분석 → 요건 확인(AskUserQuestion) → 리서치 → 계획 승인 → 실행 → 검증까지 자동 진행
```
...
| 항목 | 기존 방식 (n8n UI 수동 조작 / 즉흥적인 AI 프롬프트) | 이 킷 사용 시 |
|------|---|---|
| **요구사항 오해 방지** | AI에게 바로 "이렇게 고쳐줘"라고 하면 다르게 이해한 채로 워크플로우가 바로 바뀔 수 있음 | 실제로 손대기 전에 이해한 내용을 반드시 요약·재확인(AskUserQuestion) — 엉뚱한 변경 사고 방지 |
| **최신 문법 반영** | 사람의 기억이나 AI의 학습 데이터에만 의존 → outdated된 노드 파라미터/typeVersion 사용 위험 | 매번 context7 공식 문서 + WebSearch로 최신 스펙을 확인 후 적용 |
| **변경 안전성** | UI에서 수동으로 바꾸면 실수해도 바로 알기 어려움 | 매 변경 직후 `n8n_validate_workflow` 자동 실행, 에러 시 `n8n_autofix_workflow`로 자동 복구 시도, 실패 시 명확히 보고 |
| **감사 추적** | 무엇을 왜 바꿨는지 기록이 남지 않음 | 분석·요건·리서치·계획·실행결과가 전부 `_workspace_n8n/`에 파일로 남아 나중에 복기 가능 |
| **환경 셋업** | "n8n 어떻게 설치하더라", "이 스크립트 어디 있지" 같은 삽질 반복, 신규 팀원 온보딩에 시간 소요 | OS(Mac/Windows) 자동 감지 + 스크립트 경로 자동 탐지·캐싱으로 한 번의 요청으로 기동 |
| **이식성** | 개인 PC 절대경로가 스크립트/문서에 박혀 있어 다른 사람이 그대로 못 씀 | 프로젝트 루트/환경변수 기준으로 경로를 동적으로 해석 — 누구 PC에서든 그대로 동작 |
| **팀 지식 축적** | Code 노드 문법, expression 문법 등이 개인 머릿속에만 있음 | 로컬 n8n 스킬 7종으로 팀 공통 노하우가 프로젝트에 파일로 남아 계속 재사용됨 |
